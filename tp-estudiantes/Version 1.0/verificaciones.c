...
int buscarUltimoID(char archivoEstudiante[])
{
    ...
    if(buffer != NULL)
    {
        fseek(buffer,sizeof(stEstudiante),SEEK_END); // si el archivo existe busco el ultimo ID y le sumo 1
    ...
    {
        legajo=1; // si no existe el archivo, por defecto el legajo va a ser 1
    }
    return legajo;
}
